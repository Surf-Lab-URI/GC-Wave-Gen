function [transfo] = GenerateTransfo_Fabio_par(PixRes, PIVRes, CST)
...
surface_fab = 3034-PixRes.Combo_Surface_eta+1; %surface I need to do SU with to be
%like Kianoosh; It's a flipud of the Combo_Surface!
surface_fab_smth = 3034-PixRes.Combo_Surface_eta_smth+1; %3034 is the height in pix of a raw PIV image 
f = fft(surface_fab(xx));
fs = fft(surface_fab_smth(xx));
...
for z_index = 1:length(h)
    ...
    SU_E(z_index,:) = h(z_index)+ifft(Fs.*exp(-h(z_index)*k_LFV),'nonsymmetric'); % zeta+sum(a exp(ikx) exp(-kz))
    %smooth version of SU (to rotate the velocity)
    
    ORB_vel(z_index,:) = ifft(-1i*Fs.*om.*exp(-h(z_index)*k_LFV),'nonsymmetric'); % orbital velocity sum(-i om  a exp(ikx) exp(-kz))
...
u = transfo.SU; SU = imag(transfo.SU);
[SUH] = TransformSUField_decay_hor(u, PIVRes, SU);
transfo.SUH = SUH;
...
%% Scale factors
        ...
        u = Z; smth_param = 0.001; X = PIVRes.xPIV; Z = PIVRes.zPIV;
        [~, du_dz] = csapsDiff(u, smth_param, X, Z);
        dZ_dZeta = du_dz;
        ...
        % Zeta and columns are constant x.
            u = Xi; SU = real(transfo.SU(2:end,:));
            [uInt] = inverseTransformVelField_decay(u, PIVRes, SU);
            Xi_XZ = uInt;
            
        Xi_XZ_Smth = smoothn(Xi_XZ, 0, 'robust'); % Xi = Xi(x,z)
        Xi_XZ_Smth(~isnan(Xi_XZ)) = Xi_XZ(~isnan(Xi_XZ));
        
        u = Xi_XZ_Smth;smth_param = 0.001; X = PIVRes.xPIV; Z = PIVRes.zPIV;
        [du_dx, ~] = csapsDiff(u, smth_param, X, Z);
        dXi_dx = du_dx;
        ...
        % Transform dXi(x,z)/dx, dXi(x,z)/dz to dXi(Xi,Zeta)/dx, dXi(Xi,Zeta)/dz 
            ...
            u = dXi_dx; SU = real(transfo.SU(2:end,:));
            [uInt] = TransformVelField_decay(u, PIVRes, SU);
            dXi_dX = uInt;
            u = dXi_dX; SU = imag(transfo.SU(2:end,:));
            [uInt] = TransformVelField_decay(u, PIVRes, SU);
            dXi_dX = uInt;
        ...
        % Calculating dX(Xi,Zeta)/dXi and dX(Xi,Zeta)/dZeta 
            u = repmat(PIVRes.xPIV,size(SUR,1),1); SU = real(transfo.SU(2:end,:));
            [uInt] = TransformVelField_decay(u, PIVRes, SU);
            X_SF = uInt;
            u = X_SF; SU = imag(transfo.SU(2:end,:));
            [uInt] = TransformVelField_decay_hor(u, PIVRes, SU);
            X_SFH = uInt;
            
        X_SFH_Smth = smoothn(X_SFH, 0, 'robust');
        X_SFH_Smth(~isnan(X_SFH)) = X_SFH(~isnan(X_SFH));
        
        u = X_SFH_Smth; smth_param = 0.001; X = PIVRes.xPIV; Z = PIVRes.zPIV;
        [~, du_dz] = csapsDiff(u, smth_param, X, Z);
        dX_dZeta = du_dz;
