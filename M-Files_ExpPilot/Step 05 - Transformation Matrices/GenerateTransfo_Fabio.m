function [transfo] = GenerateTransfo_Fabio(PixRes, PIVRes, CST)
...
surface_fab=3034-PixRes.Combo_Surface_eta+1; %It's a flipud of the Combo_Surface!
...
for z_index = 1:length(h)
    ...
    ORB_vel(z_index,:) = ifft(-1i*Fs.*om.*exp(-h(z_index)*k_LFV),'nonsymmetric'); % sum(a om exp(ikx) exp(-kz))
    ORB_acc(z_index,:) = ifft(-Fs.*om.^2.*exp(-h(z_index)*k_LFV),'nonsymmetric'); % sum(a om exp(ikx) exp(-kz))
    % smooth version of the orbital velocity and acceleration at surface
