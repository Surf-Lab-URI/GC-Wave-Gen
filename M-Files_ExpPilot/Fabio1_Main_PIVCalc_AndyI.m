...
ExpPilots = [5];%[1];%[ 1 1 2 2 3 3 4 4 4 5 5 ];
...
%         end
        ...
        [PIV] = Pre_process_PIV_Image_Air(PIV); %pre_proc 1st image (scale and offset values, smooth it using Hann function)
        PIV1_A = PIV;
        PIV = PIVAir2_CamAngle;
        [PIV] = Pre_process_PIV_Image_Air(PIV); %pre_proc 2nd image(scale and offset values, smooth it using Hann function)
        PIV2_A = PIV;
...
%         end
        ...
        [PIVSurf_W2_Raw_Undistorted] = PIVSurfW_LensDistCorr(PIVSurf_W2_Raw); %For testing. Remove later
        % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % TO BE DONE!!!!!
        
        % Camera Angle Correction
        [PIVSurfW2_CamAngle] = PIVSurfWater_CamAngle_Correction(PIVSurfW2_Undistorted);
        [PIVSurfW2_Raw_CamAngle] = PIVSurfWater_CamAngle_Correction(PIVSurf_W2_Raw_Undistorted);
...
%         [BadFramePIVSurfW2, XPIVSurfW2_Surface, PIVSurfW2_Surface, XPIVW_PIVSurfW2_Surface, PIVW_PIVSurfW2_Surface, PIVW2_Surface] = ExtractSurface_PIVSurfWater(PIVSurfW2_Raw_CamAngle,PIV2_W);
        
        % Mask PIVWater2
        [Mask2_W] = PIVWater_Mask(PIV2_W, PIVW2_Surface);
%         Mask2_W(isnan(Mask2_W)) = 1;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% PIV CALCULATIONS
%        [CompVelWater] =  ComputeVelocities_Quick_NoFilt_Water_InitVelOrb(PIV1_W, PIV2_W, Mask1_W, Mask2_W, Mask1_W, IntrWndw_W, GrdSpc_W,0*Uorb_W1,0*Vorb_W1);
%        
%        figure
%        imagesc(CompVelWater.delta_z*CST.DX_W/CST.DT_W*100)
%        c = colorbar;
%        c.Label.String = 'Vertical Velocity (cm/s)';
%        hold on
%        title("pixels = " + CST.DX_W*100*4*100 + " cm " + LoadPath + "Image " + ImageNum_Water1)
%        ylim([150,300])
%        plot(XPIVW_PIVSurfW1_Surface/4, PIVW_PIVSurfW1_Surface/4, 'r')
%        set(gca,'DataAspectRatio',[1 1 1])
       
%        [CompVelWater2] =  ComputeVelocities_Quick_NoFilt_Water_InitVelOrb(PIV1_W, PIV2_W, Mask1_W, Mask2_W, Mask1_W, IntrWndw_W, GrdSpc_W,Uorb_W1,Vorb_W1);
%        IntrWndw_W3 = [128 64 32 16 8];
%        GrdSpc_W3 = [64 32 16 8 4];
%        [CompVelWater3] =  ComputeVelocities_Quick_NoFilt_Water_InitVelOrb(PIV1_W, PIV2_W, Mask1_W, Mask2_W, Mask1_W, IntrWndw_W3, GrdSpc_W3,0*Uorb_W1,0*Vorb_W1);
%        
%        
%        IntrWndw_W4 = [256 128 64 16 8];
%        GrdSpc_W4 = [128 64 32 8 4];
%        [CompVelWater4] =  ComputeVelocities_Quick_NoFilt_Water_InitVelOrb(PIV1_W, PIV2_W, Mask1_W, Mask2_W, Mask1_W, IntrWndw_W4, GrdSpc_W4,0*Uorb_W1,0*Vorb_W1);
%        
%        IntrWndw_W5 = [128 64 48 32 16 8];
%        GrdSpc_W5 = [64 32 24 16 8 4];
%        [CompVelWater5] =  ComputeVelocities_Quick_NoFilt_Water_InitVelOrb(PIV1_W, PIV2_W, Mask1_W, Mask2_W, Mask1_W, IntrWndw_W5, GrdSpc_W5,0*Uorb_W1,0*Vorb_W1);
%   
       [Uorb_W2,Vorb_W2] = GenerateTransfo_Fabio_airwater(XPIVW_PIVSurfW2_Surface, PIVW_PIVSurfW2_Surface, CST, h, w, PIV2_W);


       IntrWndw_W6 = [128 96 48 32 16 8];
       GrdSpc_W6 = [64 48 24 16 8 4];
       [CompVelWater6] =  ComputeVelocities_Quick_NoFilt_Deform_Water_InitVelOrb(PIV1_W, PIV2_W, Mask1_W, Mask2_W, IntrWndw_W6, GrdSpc_W6, Uorb_W2, Vorb_W2);
       figure
       imagesc(CompVelWater6.delta_z.*CompVelWater6.Mask*CST.DX_W/CST.DT_W*100)
       c = colorbar;
       c.Label.String = 'Vertical Velocity (cm/s)';
       hold on
       title("Deformed, pixels = " + CST.DX_W*100*4*100 + " cm " + LoadPath + "Image " + ImageNum_Water1)
       ylim([150,300])
       plot(XPIVW_PIVSurfW1_Surface/4, PIVW_PIVSurfW1_Surface/4, 'r')
       set(gca,'DataAspectRatio',[1 1 1])
%-%-%-%-%-%-%-%-%-%-% Match PIV Air coordinates
        ...
        PIV_Surface_PIVAir = PIV_Surface_PIVAir(CST.IW/2:CST.GS:end)/CST.GS;
        
        XPIV_Surface_PIVAir = XPIV_LFV_Surface(4333:8940);
        XPIV_Surface_PIVAir = XPIV_Surface_PIVAir(CST.IW/2:CST.GS:end)/CST.GS;
