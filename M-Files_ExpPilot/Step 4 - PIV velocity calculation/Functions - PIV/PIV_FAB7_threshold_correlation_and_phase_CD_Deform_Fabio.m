function [CompVel] = PIV_FAB7_threshold_correlation_and_phase_CD_Deform_Fabio(PIV1, PIV2, Mask, IntrWndw, GrdSpc)

PIV1 = double(PIV1) .* Mask;
PIV2 = double(PIV2) .* Mask;
...
Threshold = 0.4; %0.4 originally (Andy changed it)
        ...
        if VALID_BLOCK %if the PIV can be done (not masked)
           MASK(bxCNTr, bxCNTc)=1;
            ...
            [Xpky, Xpkx] = find( Xcorr == max(max(Xcorr)) ); %find max in Cross correlation
              ...
              if ( length(Xpky)==1 && length(Xpkx)==1 ) %if local calculation OK. (both phase and cross correlation agree with single peak)
                ...
                if (dcor(bxCNTr, bxCNTc)>Threshold && abs(ldelx)<IW/2 && abs(ldely)<IW/2 ) %is this correlation better than before
...
%             bxBmm = bxBmm.*Hanning2d;
            ...
            [Xpky, Xpkx] = find( Xcorr == max(max(Xcorr)) );%find max in cross correlation
               ...
               if ( length(PHpky)==1 && length(PHpkx)==1 && length(Xpky)==1 && length(Xpkx)==1 && Xpky==PHpky && Xpkx==PHpkx  ) %if local calculation OK. (both phase and cross correlation agree with single peak)
                ...
                if (dcor(bxCNTr, bxCNTc)>INTdcor(bxCNTr, bxCNTc)-Threshold/lvl && abs(ldelx)<IW/2 && abs(ldely)<IW/2 ) %is this correlation better than before
%                 if (dcor(bxCNTr, bxCNTc)>Threshold && abs(ldelx)<IW/2 && abs(ldely)<IW/2 ) %is this correlation better than before
