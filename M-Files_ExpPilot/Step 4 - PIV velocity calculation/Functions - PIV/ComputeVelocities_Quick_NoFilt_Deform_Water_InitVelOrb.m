function [CompVel] =  ComputeVelocities_Quick_NoFilt_Deform_Water_InitVelOrb(PIV1, PIV2, Mask1, Mask2, IntrWndw, GrdSpc, Uorb_W2, Vorb_W2)
...
Uorb_W2 = (smoothn(Uorb_W2,'robust'));
Vorb_W2 = (smoothn(Vorb_W2,'robust'));


IM1_D=PIV1;
IM2_D=interp2(1:size(PIV2,2),(1:size(PIV2,1))',PIV2,X1+Uorb_W2,Y1+Vorb_W2,'*linear'); %PIV2
...
%         end
%         [X,Y] = meshgrid(x, y);
%         pdelx = Uguess; %zeros(bxsNh, bxsNw);
%         pdely = Vguess; %zeros(bxsNh, bxsNw);
%         U1 = interp2(X,Y,pdelx,X1,Y1,'*spline'); 
%         V1 = interp2(X,Y,pdely,X1,Y1,'*spline');
...
delx = NaN(bxsNh, bxsNw);% pdelx; % Total velocity in x - accumulates from previous levels with INTdel and gdel
dely = NaN(bxsNh, bxsNw); %pdely % Total velocity in y - accumulates from previous levels with INTdel and gdel
...
CompVel.Mask = MASK;
 ...
 for i = 1:length(x)
        for ii = 1:length(y)
            delx(ii, i) = delx(ii, i) + mean(mean(Uorb_W2(y(ii)-IW/2+1:y(ii)+IW/2-1,x(i)-IW/2+1:x(i)+IW/2-1),'omitnan'),'omitnan');
            dely(ii, i) = dely(ii, i) + mean(mean(Vorb_W2(y(ii)-IW/2+1:y(ii)+IW/2-1,x(i)-IW/2+1:x(i)+IW/2-1),'omitnan'),'omitnan');
        end
 end
