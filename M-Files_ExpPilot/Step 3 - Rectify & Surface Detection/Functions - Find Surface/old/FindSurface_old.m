function [imSurf] = FindSurface_old(img, Step,S,expName)
...
%     [~,locs] = findpeaks(gv1, 'minpeakheight', max(gv1)/4, 'npeaks',1);
    ...
    [~,locs] = findpeaks(gv1, 'minpeakheight', max(gv1)/2.5, 'MinPeakProminence', 1e-2); %, 'npeaks',1);
    
    if str2double(expName)>36
        [~,locs] = findpeaks(gv1, 'minpeakheight', max(gv1)/2, 'MinPeakProminence', 1e-2); %, 'npeaks',1);
    end
    ...
    %if ~ismember(str2double(expName),HighWindExp)
    if i>700 && i<9800 && ~isempty(locs)
        if (abs(surface2(i-1)-surface2(i))>Jump_thr && ~ismember(str2double(expName),HighWindExp)) || abs(surface2(i-1)-surface2(i))>Jump_thr*1.2
            [~,locs] = findpeaks(gv1, 'minpeakheight', max(gv1)/4); %, 'MinPeakProminence', 1e-4);
            [~,Ind] = min(abs(surface2(i-1)-locs));
            surface2(i) = locs(Ind);
            surface3(i) = s_gv2+locs(Ind)-Step-1;
            if abs(surface2(i-1)-surface2(i))>Jump_thr*1.2
                L = 1000;
...
surf2_corr = despike_jumps(surface3,Jump_thr,MarkInt,'PIVSURF');
surface3 = surf2_corr;
