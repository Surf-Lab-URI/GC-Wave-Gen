function [XPos,YPos,PIVSurf_PIVMatch,XPIV_PIVSurf_Surface,PIVSurf_Surface,PIV_match_Surface,BadFramePIVSurf] = ExtractSurface_PIVSurf_old(PIVSURF_CR1,PIV1,expName)
...
Xgood = 1501:9300; % good points where image is still bright enough to detect surface
X = Xgood;
...
X_PIVSURF2 = X+XPos(1);
XPIV_PIVSurf_Surface = round(X_PIVSURF2(1)):round(X_PIVSURF2(end)); % rounded coordinates 
...
if str2double(expName)>12 && str2double(expName)<25
    ...
    Rot = MM*[XPIV_PIVSurf_Surface;PIVSurf_Surface];
...
% Case 4: w.d. ~70 cm ---> expt 37:48, no corrections
