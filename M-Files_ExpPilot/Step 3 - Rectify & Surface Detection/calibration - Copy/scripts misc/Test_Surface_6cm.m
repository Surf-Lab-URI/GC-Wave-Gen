%%% Test surface for 6 cm depth (tests 06092022)
clear
close all
clc
...
for ii = 13:24
    ...
    for i = 0:2:114
        ...
        %% Load PIVSURF and reconstruct rectified image
        ...
        X = 1:length(imSurf_PIVSURF.surface);
...
%         PIVSurf_Surface_resized = interp1(PIVSurf_Surface_resized2(:,1),PIVSurf_Surface_resized2(:,2),1:length(PIV_Surface));
        ...
        %% Further rotation to correct flat surface %----- NO FOR h = 6 cm -----%
...
%         xr{i+1} = interp1(Xr(1,:),Xr(2,:),1:5924,'pchip','extrap');
        xr{i+1} = interp1(Xr(1,:),Xr(2,:),round(X_PIVSURF2(1)):round(X_PIVSURF2(end)),'pchip','extrap');
        xr2{i+1} = interp1(1:5950/5924:5950,xr{i+1},1:5924);
        X_PIVSURF = round(X_PIVSURF2(1)):round(X_PIVSURF2(end));
        %     figure;plot(PIV_Surface,'r');hold on;plot(xr-DX,'b');plot(xr2-DX,'g')
        %     figure;plot(xr-DX-PIV_Surface,'b');hold on;plot(xr2-DX-PIV_Surface,'g');
        ...
        X_LFV = LFV_Surface_resized_PIVSURF2(:,1)+XPos(1);
        LFV_Surface_resized = LFV_Surface_resized_PIVSURF2(:,2)+YPos(1);
