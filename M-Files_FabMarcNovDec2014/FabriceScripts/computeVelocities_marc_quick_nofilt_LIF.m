...
function compVel =  computeVelocities_marc_quick_nofilt_LIF(IM1, IM2, mask1, mask2, IntrWndw, GrdSpc)
% [delx_int dely_int, MASK,cor_mtx,level_mtx]
% tic,
%IM1 = pivIn.IM_a;IM2 = pivIn.IM_b;


IM1f=medfilt2(IM1,[5 5]);
IM2f=medfilt2(IM2,[5 5]);
...
%           VALID_BLOCK = ~isnan(sum(sum(mask(bdryT:bdryB,bdryL:bdryR)))); %if no NANs at all in subwindow
        ...
        if VALID_BLOCK %if the PIV can be done (not masked)
            ...
            try
                ...
                CONCa(bxCNTr, bxCNTc)=mean(mean(IM1f( (bdryT:bdryB) - backy, (bdryL:bdryR) - backx )));
                CONCb(bxCNTr, bxCNTc)=mean(mean(IM2f( (bdryT:bdryB) + forwardy, (bdryL:bdryR) + forwardx )));
