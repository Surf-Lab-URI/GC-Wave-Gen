...
% 05/08/2013
%%
function compVel =  computeVelocities_LfvOrb1(exp_name, image_pair_number, IM1, IM2, mask1, mask2, delxOrb, delyOrb)
        ...
        for r = y %loop in row (y, coordinate in image)
            ...
            if VALID_BLOCK %if the PIV can be done (not masked)
                ...
                % check that flow points within the boundary of 2nd image
                    ...
                    %
                    if ( length(PHpky)==1 && length(PHpkx)==1 && length(Xpky)==1 && length(Xpkx)==1 && Xpky==PHpky && Xpkx==PHpkx && ~isnan(sum(sum((Xcorr))))  ) 
                    ...
                    else %no local calculation possible -> keep global velocity and correlation from previous level
...
compVel.mask = MASK;
%
compVel.exp_name = exp_name;
compVel.im_pair_num = image_pair_number;
