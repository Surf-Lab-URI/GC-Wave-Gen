function compVelFab = PIV_FAB6_PivsurfOrb1 (IM1,IM2, mask1, mask2, delxOrb, delyOrb, IntrWndw, GrdSpc, iws)

%%
orbStep = GrdSpc(1)/iws;
delxOrb2 = flipud(delxOrb);
delyOrb2 =flipud(delyOrb);
delxOrb = flipud(delxOrb(orbStep:orbStep:end-orbStep,orbStep:orbStep:end-orbStep)); % field of orbitals to initialize piv routine
delyOrb = flipud(delyOrb(orbStep:orbStep:end-orbStep,orbStep:orbStep:end-orbStep));
IM1 = fliplr(IM1);
IM2 = fliplr(IM2);
mask1 = fliplr(mask1);
mask2 = fliplr(mask2);
...
%     keyboard
    if lvl==1
        if (size(delxOrb)~=size(delx)) %|| size(delyOrb)~=size(dely)
            keyboard
        end
    end
    ...
    if lvl==1 %initialize global motion (and correlation) if it's 1st level, otherwise it's passed from previous level (after beeing interpolated to the proper grid)
        INTdelx = delxOrb; % replaces:  INTdelx=zeros(bxsNh, bxsNw);
        INTdely = delyOrb; % replaces:  INTdely=zeros(bxsNh, bxsNw);
        ...
        for r = y %loop in row (y)
            ...
            if VALID_BLOCK %if the PIV can be done (not masked)
                ...
                if ( (bdryT + gdely_round )>=1 && (bdryB + gdely_round)<=h && ...
                    ...
                    if ( length(PHpky)==1 && length(PHpkx)==1 && length(Xpky)==1 && length(Xpkx)==1 && Xpky==PHpky && Xpkx==PHpkx  && ~isnan(sum(sum((Xcorr))))) %if local calculation OK. (both phase and cross correlation agree with single peak)
...
compVelFab.delxOrb = delxOrb2;
compVelFab.delyOrb = delyOrb2;
