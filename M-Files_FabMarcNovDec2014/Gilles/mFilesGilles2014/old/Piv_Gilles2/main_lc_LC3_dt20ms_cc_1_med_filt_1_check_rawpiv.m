...
clear all
close all
lst = dir('F:\data\ExpLC3_dt20ms_cc_1\ComputedVelocities\Exp*');
for i=1:length(lst)
%     i = 1
     load(['F:\data\ExpLC3_dt20ms_cc_1\ComputedVelocities\' lst(i).name]);
     hFig = figure;
     set(hFig,'NumberTitle','off', 'Name', lst(i).name(end-15:end-12));
     imagesc(curl(compVel.delx_int.*compVel.MASK,compVel.dely_int.*compVel.MASK)), colorbar, caxis([- max(abs(caxis)) +max(abs(caxis))])
end
