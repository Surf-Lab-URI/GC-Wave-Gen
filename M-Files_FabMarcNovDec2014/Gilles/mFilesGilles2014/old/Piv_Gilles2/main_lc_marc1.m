...
image_pair_number = 1150
    ...
    Orbitals = calcOrbitals_lc_lfv(surfLfv,imgLfv_t_cut,vResLfv,hResLfv,pivRes,iws,deltaT,pivResReal);
    Orbitals_interp = interpOrbitals_lc_lfv(surf1,pivRes,Orbitals.u,Orbitals.w,iws,lim);
    %% Calcul de la PIV
    delxOrb1 = Orbitals_interp.u(1:end-1,1:end-3);
    delxOrb = flipud(delxOrb1);
    delzOrb1 = Orbitals_interp.w(1:end-1,1:end-3);
    delzOrb = flipud(delzOrb1);
    IM11 = fliplr(IM1);
    IM22 = fliplr(IM2);
    mask11 = fliplr(mask1);
    mask22 = fliplr(mask2);
    compVel_orb2 = PIV_FAB6_LfvOrb1 (IM11,IM22, mask11, mask22, delxOrb, delzOrb);
% end
