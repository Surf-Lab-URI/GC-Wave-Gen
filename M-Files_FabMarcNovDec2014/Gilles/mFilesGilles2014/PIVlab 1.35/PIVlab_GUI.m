...
Commandline checken!
OK change tutorial link to blog
OK Wiener2 denoise preprocessing filter
OK Reference to Damien Garcia
OK Include parameter name as an option when colorbar is displayed
OK Hint during LIC: If this stays here for x time, have a look at the output in MATLABs command window
OK Load Images button bigger
OK In save to workspace: explain what u_filtered / smoothed etc mean.
OK Export as PDF
OK Add Bibtex key for my thesis and also for the blog: (AND ALSO FOR ALGORITHMS PAPER...!
Load masks from another session
piv_FFTmulti (line 122) --> You might add "~isempty(x) && ~isempty(y)" in that line to solve that problem.
plot correlation result in a Popup window?
subtract a flow field: Make sure that it has the same size, and give information on the size, e.g. shoulb be 8x12 but is 10x12.
colored vectors (I think there was a good submission in FEX)
OK state that shear rate is SIMPLE SHEAR!!
...
OK state that minimum version is MATLAB Version 7.10 (R2010a)
%}
