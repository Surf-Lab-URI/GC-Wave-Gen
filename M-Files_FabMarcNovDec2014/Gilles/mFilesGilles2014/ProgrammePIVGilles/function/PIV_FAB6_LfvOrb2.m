function compVelFab = PIV_FAB6_LfvOrb2 (IM1,IM2, mask1, mask2, delxOrb, delyOrb, IntrWndw, GrdSpc);

IM1=double(IM1) .* ~isnan(mask1);
IM2=double(IM2) .* ~isnan(mask2);
...
for lvl = 1:number_of_levels-1; %First level
    ...
    if lvl==1 %initialize global motion (and correlation) if it's 1st level, otherwise it's passed from previous level (after beeing interpolated to the proper grid)
        INTdelx = delxOrb(orbStep:orbStep:end,orbStep:orbStep:end) ; % replaces:  INTdelx=zeros(bxsNh, bxsNw);
        INTdely = delyOrb(orbStep:orbStep:end,orbStep:orbStep:end);
%         keyboard
        INTdelx = INTdelx(1:bxsNh, 1:bxsNw) ; % replaces:  INTdelx=zeros(bxsNh, bxsNw);
        INTdely = INTdely(1:bxsNh, 1:bxsNw) ;  % replaces:  INTdely=zeros(bxsNh, bxsNw);
...
% %            Choix du probleme a gauche ou centre
%             if VALID_BLOCK %if the PIV can be done (not masked)
%                 MASK(bxCNTr, bxCNTc)=1;
%                 if ( (bdryT + gdely_round )>=1 && (bdryB + gdely_round)<=h && ...
%                         (bdryL + gdelx_round )>=1 && (bdryR + gdelx_round)<=w && ~isnan(mask2(r+gdely_round, c+gdelx_round)) ) % check that flow points within the boundary of 2nd image
...
%                     Hanning2d=0.5*(1-cos(2*pi*[0:Ny-1]'/(Ny-1)))*0.5*(1-cos(2*pi*[0:Nx-1]/(Nx-1)));
            ...
            forwardx=ceil(gdelx_round/2);
            forwardy=ceil(gdely_round/2);
            backx=floor(gdelx_round/2);
            backy=floor(gdely_round/2);
            
            if VALID_BLOCK %if the PIV can be done (not masked)
                MASK(bxCNTr, bxCNTc)=1;
                if ( (bdryT + forwardy )>=1 && (bdryB + forwardy)<=h && ...
                        (bdryL + forwardx )>=1 && (bdryR + forwardx)<=w && ...
                        (bdryT - backy )>=1 && (bdryB - backy)<=h && ...
                        (bdryL - backx)>=1 && (bdryR - backx)<=w) % check that flow points within the boundary of 2nd image
                    ...
                    if ( length(PHpky)==1 && length(PHpkx)==1 && length(Xpky)==1 && length(Xpkx)==1 && abs(Xpky-PHpky)<2 && abs(Xpkx-PHpkx)<2  && ~isnan(sum(sum((Xcorr))))) %if local calculation OK. (both phase and cross correlation agree with single peak)
                        ldelx =  Xpkx - IW/2 - 1; %local velocity calculated here
                        ldely = Xpky - IW/2 - 1;
                        dcor(bxCNTr, bxCNTc) = Xcorr(Xpky, Xpkx); %local correlation calculated here
                        if (dcor(bxCNTr, bxCNTc)>0 && abs(ldelx)<IW/2 && abs(ldely)<IW/2 ) 
                            ...
                            if ((Xpkx <= Xcorr_x-1) && (Xpky <= Xcorr_y-1) && (Xpkx >= 2) && (Xpky >= 2)) %is subpix interpolation possible (enough room around peak?)
                                ...
                                if (isreal([SubpixelY SubpixelX]) && abs(SubpixelY)<1 && abs(SubpixelX)<1) %valid subpixel
                                    ...
                                    dely(bxCNTr, bxCNTc) = gdely + ldely; %should try to use gdelx_round + ldelx instead
                                end
    ...
    reject_cor=(dcor<corr_threshold_at_level);
    delx(reject_cor==1)=NaN;
    dely(reject_cor==1)=NaN;
    ...
    reject_median=reject_median(b+1:(end-b), b+1:(end-b) );
    ...
    reject_total=sum(sum(reject_cor==1));
    replace_total=sum(sum(reject_median==1));
    reject_total_percent(lvl)=reject_total/(I*J)*100;
    replace_total_percent(lvl)=replace_total/(I*J)*100;
...
reject_total=sum(sum(reject_cor==1));
replace_total=sum(sum(reject_median==1))+sum(sum(reject_dynamic_u==1))+sum(sum(reject_dynamic_v==1));
replace_total_percent(lvl)=replace_total/(I*J)*100;
reject_total_percent(lvl)=reject_total/(I*J)*100;
...
dely_z=dely;
dcor_z=dcor;
i=isnan(delx+dely+dcor);delx_z(i)=NaN;dely_z(i)=NaN;dcor_z(i)=0;
[delx_int,s,exitflag,wtot]=(smoothn(delx_z,dcor_z,0,'robust'));
[dely_int,s,exitflag,wtot]=(smoothn(dely_z,dcor_z,0,'robust'));


[delx_ints,s,exitflag,wtot]=(smoothn(delx_z,dcor_z,'robust'));
[dely_ints,s,exitflag,wtot]=(smoothn(dely_z,dcor_z,'robust'));

compVelFab.delx_int = delx_int;
compVelFab.dely_int = dely_int;
compVelFab.delx_ints = delx_ints;
compVelFab.dely_ints = dely_ints; 
compVelFab.MASK = MASK;
compVelFab.dwhatlevel = dwhatlevel;
compVelFab.dcor = dcor;
compVelFab.delxOrb =delxOrb;
compVelFab.delyOrb =delyOrb;
