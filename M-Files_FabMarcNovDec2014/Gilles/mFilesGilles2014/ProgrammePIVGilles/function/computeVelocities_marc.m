...
% 11/14/2013
%%
function compVel =  computeVelocities_marc(IM1, IM2, mask, IntrWndw, GrdSpc)
        ...
        for r = y %loop in row (y, coordinate in image)
            ...
            if VALID_BLOCK %if the PIV can be done (not masked)
                ...
                if ( (bdryT + forwardy )>=1 && (bdryB + forwardy)<=h && ...
                    ...
                    if ( length(PHpky)==1 && length(PHpkx)==1 && length(Xpky)==1 && length(Xpkx)==1 && abs(Xpky-PHpky)<2 && abs(Xpkx-PHpkx)<2 )
