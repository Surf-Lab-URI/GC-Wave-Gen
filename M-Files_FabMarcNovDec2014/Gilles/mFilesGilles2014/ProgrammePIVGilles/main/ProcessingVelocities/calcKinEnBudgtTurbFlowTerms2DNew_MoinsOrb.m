...
for image_pair_number=start_image_number:end_image_number
    ...
    load(['\\beo\data\Exp' exp_name '\ProcessedVelocities\Exp' exp_name '_MOTDec' sprintf(['%0' num2str(num_of_digits) 'd'], image_pair_number-1)]);
    U(:,1)=MOTDec.uMoinsUorbm_rs; % U(i,j,t), t=1==n-1, t=2 ==n, t=3==n+1
    V(:,1)=MOTDec.vMoinsVorbm_rs;
    u(:,:,1)=MOTDec.up_rs;
    v(:,:,1)=MOTDec.vp_rs;
    load(['\\beo\data\Exp' exp_name '\ProcessedVelocities\Exp' exp_name '_MOTDec' sprintf(['%0' num2str(num_of_digits) 'd'], image_pair_number)]);
    U(:,2)=MOTDec.uMoinsUorbm_rs; 
    V(:,2)=MOTDec.vMoinsVorbm_rs;
    u(:,:,2)=MOTDec.up_rs;
    v(:,:,2)=MOTDec.vp_rs;
    load(['\\beo\data\Exp' exp_name '\ProcessedVelocities\Exp' exp_name '_MOTDec' sprintf(['%0' num2str(num_of_digits) 'd'], image_pair_number+1)]);
    U(:,3)=MOTDec.uMoinsUorbm_rs; 
    V(:,3)=MOTDec.vMoinsVorbm_rs;
    u(:,:,3)=MOTDec.up_rs;
    v(:,:,3)=MOTDec.vp_rs;
    clear ReyDec
...
% Sauvegarde
filename = ['Exp' exp_name '_KinEnBudgtTurbFlow_MoinsOrb'];
outfile = [save_path filename];
save(outfile, 'KEB')
