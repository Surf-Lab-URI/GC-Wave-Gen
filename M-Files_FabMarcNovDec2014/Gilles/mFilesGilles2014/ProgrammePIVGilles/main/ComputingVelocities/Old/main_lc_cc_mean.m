...
gravity=9.81; %m/s^2
gravity_piv=gravity/ResPivReal*DeltaT^2;
num_of_digits = 4;
...
%     Lfv_LD = correctLfvLensDist_lc(Lfv);  % Raid en panne...
Lfv_LD = Lfv;
    Lfv_cut=Lfv_LD(401:1600,401:1600); % Seul une partie de l'image est expoitable
    surfLfv=findSurface_lc(Lfv_cut); 
    
    %% Calcul de la PIV
    surf1.z_s_f(1:length(surf1.z_s_f))=mean(surf1.z_s_f);
    surf2.z_s_f(1:length(surf2.z_s_f))=mean(surf2.z_s_f);
    
    SU1 = SUcc(surfLfv, surf1, ResPiv, hResLfv, vResLfv, GrdSpc(end), 7);
    SU2 = SUcc(surfLfv, surf2, ResPiv, hResLfv, vResLfv, GrdSpc(end), 7);
