            ...
            if VALID_BLOCK % Whether the PIV can be done (not masked)
                try
                    ...
                    % 3 Point Gaussian Interpolation
                     ...
                     if isreal([SubpixelY SubpixelX]) && (SubpixelY<1) && (SubpixelX<1) &&  ldelx< IW/2 &&  ldely< IW/2 %&& max(max(Xcorr))>0.5
    ...
    for r = y % Loop in row: y-coordinate of center of interrogation window
        ...
        if VALID_BLOCK % Whether the PIV can be done (not masked)
            ...
            try
                ...
                if isreal([SubpixelY SubpixelX]) && (SubpixelY<1) && (SubpixelX<1) %&& max(max(Xcorr))>0.4
...
U1 = interp2(X,Y,smoothn(delx,'robust'),X1,Y1,'*spline'); 
V1 = interp2(X,Y,smoothn(dely,'robust'),X1,Y1,'*spline');
