%% Compute instantaneous mean velocity
...
for i = 3%:length(ExpDir) % Loop on the number of experiments
    ...
    for ii = 1%[1,2,9:16]% 1:numel(RunDir) % Loop on the number of Runs
        ...
        for idx = 1:numel(DirA) % Main Loop
            ...
            load([SaveCartAirPath DirA(idx).name]);
            load([SaveCartWaterPath DirW(idx).name]);
            
            Idx = str2double(PairNum)+1;
            MeanVelAir(:,Idx) = mean(Cartesian_Air.delx.*Cartesian_Air.Mask,2,'omitnan');
            MeanVelWater(:,Idx) = mean(Cartesian_Water.delx.*Cartesian_Water.Mask,2,'omitnan');
            Time = Idx/15;
...
%             end
        ...
        save MeanVel_in_time MeanVelAir MeanVelWater Idx PairNum Time
