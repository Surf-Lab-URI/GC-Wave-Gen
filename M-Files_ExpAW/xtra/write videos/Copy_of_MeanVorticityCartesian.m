%% Compute instantaneous mean vorticity
...
for i = 3%:length(ExpDir) % Loop on the number of experiments
    ...
    for ii = 1%[1,2,9:16]% 1:numel(RunDir) % Loop on the number of Runs
        ...
        DirA = dir([SaveCartDiffAirPath '*.mat']);
        DirW = dir([SaveCartDiffWaterPath '*.mat']);
        
        %% Processing frames
        
        for idx = 1:numel(DirA) % Main Loop
            
            PairNum = DirA(idx).name(end-7:end-4);
            load([SaveCartDiffAirPath DirA(idx).name]);
            load([SaveCartDiffWaterPath DirW(idx).name]);
            
            Idx = str2double(PairNum)+1;
            MeanVortAir(:,Idx) = mean(cartDiffAir.Vorticity.*cartDiffAir.Mask,2,'omitnan');
            MeanVortWater(:,Idx) = mean(cartDiffWater.Vorticity.*cartDiffWater.Mask,2,'omitnan');
            Time = Idx/15;
            
%             if (round(idx/numel(DirA)*100,2))/10 == round(round(idx/numel(DirA)*100,2)/10)
                disp(['Computing ' num2str(round(idx/numel(DirA)*100,2)) '%...'])
%             end
        end
        
        save MeanVort_in_time MeanVortAir MeanVortWater Idx PairNum Time
