...
% % %         NameVideo = [expName runName '_quiver_faster'];
% % %         Type = 'Air';
% % %         v = VideoWriter([SaveVideoINTdelx NameVideo,'_' Type '.avi']); % apri il file video ('FrameRate',30)
% % %         v.Quality = 95;
% % %         v.FrameRate = 25;
% % %         open(v)
        
        for idx = 889%Idx1:1:Idx2 %1:numel(DirA_Vort) %---- MAIN LOOP ----%
            ...
            for j = 2%1:2
                ...
                Ximg = (0:size(Combo_PIV.(FIELD),2)-1)*CST.DX*4;
                Zimg = fliplr(((0:size(Combo_PIV.(FIELD),1)-1)-(size(Combo_PIV.(FIELD),1)+1-mean(Combo_PIV.PIV_Surface_PIVAir)))*CST.DX*4);
                %     eval(['imagesc(Combo_PIV.' FIELD ';'])
                [MaskA] = PIVAir_Mask(Combo_PIV.INTdelx, Combo_PIV.PIV_Surface_PIVAir);
                Nstep = 15;
                Ir = 1:Nstep:size(Combo_PIV.INTdelx,1);
                Ic = 1:Nstep:size(Combo_PIV.INTdelx,2);
                MaskW = nan(size(Combo_PIV.INTdelx));
                MaskW(isnan(MaskA)) = 1;
                f=figure(1);
                f.Position = [100 100 1240 840];
                f.Position = [100 100 1240 840];
                quiver(Ximg(Ic),Zimg(Ir),Combo_PIV.INTdelx(Ir,Ic).*MaskA(Ir,Ic),Combo_PIV.INTdelz(Ir,Ic).*MaskA(Ir,Ic),1.75,'Color','r')
                hold on
                plot(Ximg,((size(Combo_PIV.(FIELD),1)-Combo_PIV.PIV_Surface_PIVAir)-mean(size(Combo_PIV.(FIELD),1)+1-Combo_PIV.PIV_Surface_PIVAir))*CST.DX*4,'Color',[0.5 0.5 0.5],'LineWidth',1.25)
                quiver(Ximg(Ic),Zimg(Ir),Combo_PIV.INTdelx(Ir,Ic).*MaskW(Ir,Ic),Combo_PIV.INTdelz(Ir,Ic).*MaskW(Ir,Ic),2,'Color','b')
                ElapsedTime = '00.00';
                ElapsedTime(1:length(num2str(round(str2double(PairNum)/15,2)))) = num2str(round(str2double(PairNum)/15,2));
                title(['Elapsed time: ' num2str(ElapsedTime) ' s'],'FontName','Times New Roman','FontSize',24);
            ...
            disp(['Computing... ' num2str(round(idx/(Idx2-Idx1),2)) '%']);
