...
for i = [6 7]%:length(ExpDir) % Loop on the number of experiments
    ...
    for ii = 1%numel(RunDir) % Loop on the number of Runs
        ...
        PIVAirFileName = [ expName '_' runName '_CST'];
        save([AirPath PIVAirFileName '.mat'], '-fromstruct',CST)
        CSTfilename = "CST.mat";
        save(CSTfilename,'-fromstruct',CST)
tic
        parfor idx = 1:1000%1000:1500%720/2%numel(image_index) % Main Loop15
...
toc
        ...
        % 
        % % Insert name script
        % Script = 'Fabio1-Main_PIVCalc-par-SPRAY4.m';
        % send_email('fabio.addona1@gmail.com',psw,'smtp.gmail.com', obj,['SCRIPT ' Script ' Experiment' expName ' ' runName ' concluded!'])
