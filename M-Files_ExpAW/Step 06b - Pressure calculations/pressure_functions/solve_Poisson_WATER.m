function Out = solve_Poisson_WATER(f,Surf,x,z,P_x,P_z,curved,periodic,Ptop)
% ---------------------------------------------------------------------- %
% --------------------------- Start function --------------------------- %
% ---------------------------------------------------------------------- %
...
surf_max_index = max(surf_index);

% Cut out columns below surface containing only nan
z = z(1:surf_max_index);
f = f(:,1:surf_max_index);
X = X(:,1:surf_max_index);
Z = Z(:,1:surf_max_index);
% % % Shift so min of surf_index is 0 to match new z above
% % surf_index = surf_index - surf_max_index;
z_num = length(z);

% Check that f is defined everywhere along the water surface, and if not
% adjust by taking surface to be at index below
for i = 1:x_num
    if isnan(f(i,surf_index(i)-1))
        surf_index(i) = surf_index(i)-1;
        fprintf('Warning Missing f value below surface at (%d, %d) \n',i, surf_index(i) + surf_max_index);
    end
end
...
if ~periodic
    ...
    pType(x_num2, :) = -1;
    for i = 1:x_num
        pType(i+1, surf_index(i)+1:end) = -1;
    end
        ...
        % number of points below boundary to include in check. Lower numbers
        %make runtime faster at risk of missing boundary points for steep waves
        for i = 2:x_num+1
            for k = surf_index(i-1):surf_index(i-1) + check - 1
                if pType(i,k) ~= -1
    ...
    pType(1, 1:surf_index(1)) = 1;

    % right boundary
    pType(x_num, 1:surf_index(end)) = 3;

    % bottom boundary
    pType(1:x_num, 1) = 4;

    %corners
    pType(1, surf_index(1)) = 5;
    pType(x_num, surf_index(end)) = 6;
    pType(x_num, 1) = 7;
    pType(1, 1) = 8;
end
...
%                 '''
            ...
            elseif pType(i,k) == 27
                try 
                    ...
                    %Neumann Condition Method Morton
                    [rowApp, colApp, coefs, const, Coef] =...
                        coef_BRCornerV2_Morton_WATER(x,z,Surf,i,k,P_x,P_z,pType,periodic);
...
%                 '''
            ...
            elseif pType(i,k) == 28
                try
                    ...
                    try
                    [rowApp, colApp, coefs, const, Coef] =...
                        coef_BLCornerV2_Morton_WATER(x,z,Surf,i,k,P_x,P_z,pType,periodic);
                    catch
                        keyboard
                    end
                    row = [row;repmat(Ind(i,k),length(rowApp),1)];
