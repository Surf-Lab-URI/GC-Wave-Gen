...
%   NOTE: If we want the velocity at the surface exactly, we start from first row
%
%   SU = imag(transfo.SU(2:end,:)); if output starting OVER the surface
%   SU = imag(transfo.SU(1:end,:)); if output starting AT the surface
%   u  = TransformVelField_decay_hor(uInt, PIVRes, SU);
%   SUH = TransformSUField_decay_hor(imag(transfo.SU(2:end,:)), PIVRes, SU); if output starting OVER the surface
%   SUH = TransformSUField_decay_hor(imag(transfo.SU(1:end,:)), PIVRes, SU); if output starting AT the surface
