function [uInt] = inverseTransformVelField_decay(u, PIVRes, SU)
...
%  But if we want to include the surface orbital velocity, we can make it
%  start from the first row.
%
%   u = uInt; (surface at the top when using imagesc)
%   SU = real(transfo.SU(2:end,:)); if output starting OVER the surface
%   SU = real(transfo.SU(1:end,:)); if output starting AT the surface


GS = PIVRes.GS;
SU=(length(PIVRes.zPIV)-(3088-SU+1)/GS) +1; %length(PIVRes.zPIV) == size(u,1);
% (3088-SU+1) is right side up
% (3088-SU+1)/GS is right side up in PIV Air
% length(PIVRes.zPIV)-(3088-SU+1)/GS) +1 is up side down in PIV Air
zPIV = PIVRes.zPIV/GS;
uInt = NaN(size(SU));
