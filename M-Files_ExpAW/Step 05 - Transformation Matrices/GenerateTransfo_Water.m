function [transfo] = GenerateTransfo_Water(PixRes, PIVRes, CST)
...
xx = 1:length(PixRes.PIVW_PIVSurfW1_Surface); % PIVSurf Water
X = find(ismember(PixRes.XPIVW_PIVSurfW1_Surface(xx), PIVRes.xPIV)); % Finds indices that are mem-
% bers of PIVSurf Water and PIVRes.xPIVV; it'll be longer than PIVsurf by a bit to avoid edge effects in SU.
% PixRes.XPIVW_PIVSurfW1_Surface x-location (with respect to PIVWater) of the PIVSurf Water surface
% PIVRes.xPIV = 4, 8, etc... are the x position of the PIV estimates


surface_fab = 3087-PixRes.PIVW_PIVSurfW1_Surface(xx)+1; %It's a flipud of the PIVW_PIVSurfW1_Surface!   %Fits perfectly with imagesc(flipud(PIV1_W))
surface_fab_smth = 3087-PixRes.PIVW_PIVSurfW1_Surface_smth(xx)+1; % 3087 is the height in pix of a raw PIV Water image 
f = fft(surface_fab);
...
%%% Notice that this is true in water: transfo.ORBX = imag(ORB_vel)  in the air
...
%% Scale factors
        ...
        dZ_dZeta(isnan(SUHR)) = SUHR(isnan(SUHR)); %Change of sign because water is upside down
        %%% dZ_dZeta has opposite sign because we are considering the water
        %%% side
        ...
        % Zeta and columns are constant x.
            ...
            % % % Xi_XZ = inverseTransformVelField_decay(Xi, PIVRes, real(transfo.SU(2:end,:))); % Xi(x,z) THIS IS THE CORRECT ONE FOR AIR
            Xi_XZ = inverseTransformVelField_decay_WATER(Xi, PIVRes, real(transfo.SU(2:end,:))); % Xi(x,z)
        ...
        % Transform dXi(x,z)/dx, dXi(x,z)/dz to dXi(Xi,Zeta)/dx, dXi(Xi,Zeta)/dz 
            dXi_dX = TransformVelField_decay_WATER(dXi_dx, PIVRes, real(transfo.SU(2:end,:)), 'linear');%'spline"
        ...
        % Calculating dX(Xi,Zeta)/dXi and dX(Xi,Zeta)/dZeta 
            X_SF = TransformVelField_decay_WATER(X, PIVRes, real(transfo.SU(2:end,:)), 'spline'); % X(x,Zeta)
