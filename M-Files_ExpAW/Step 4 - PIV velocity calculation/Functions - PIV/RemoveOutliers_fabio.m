function [Cartesian] = RemoveOutliers_fabio(CompVel,Threshold,BadSubpix)
...
ThrX = (delx-CompVel.INTdelx)>BadSubpix | (delx-CompVel.INTdelx)<-BadSubpix;
ThrZ = (delz-CompVel.INTdelz)>BadSubpix/2 | (delz-CompVel.INTdelz)<-BadSubpix/2;
delx(ThrX) = NaN; % NaN outliers
delz(ThrZ) = NaN; % NaN outliers
np = sum([ sum(abs(delx-CompVel.INTdelx)>BadSubpix,'all') , sum(abs(delz-CompVel.INTdelz)>3,'all')]);
