function [CompVel] =  ComputeVelocities_Quick_NoFilt_Deform_Water_InitVel(PIV1, PIV2, Mask1, Mask2, IntrWndw, GrdSpc, UGuess, VGuess, subpixel)
...
PIV1 = interp2(1:size(PIV1,2),(1:size(PIV1,1))',PIV1,X1-UGuess/2,Y1-VGuess/2,'*linear');
PIV2 = interp2(1:size(PIV2,2),(1:size(PIV2,1))',PIV2,X1+UGuess/2,Y1+VGuess/2,'*linear');
...
for lvl = 1:number_of_levels-1 % First level
        ...
        for r = y % Loop in row: y-coordinate of interrogation window
            ...
            if VALID_BLOCK % Whether the PIV can be done (not masked)
                try
                    ...
                    % if r > 725 && r < 775 && c >3100 && c < 3400 && lvl == 4
                    %      disp(lvl)
                    % end
                    ...
                    % 3 Point Gaussian Interpolation
                        ...
                        if ldelx< IW/2 &&  ldely< IW/2 && max(max(Xcorr)) > 0
                            delx(bxCNTr, bxCNTc) = pdelx(bxCNTr, bxCNTc) + ldelx;                          
                            dely(bxCNTr, bxCNTc) = pdely(bxCNTr, bxCNTc) + ldely;
                            %GLOBAL(bxCNTr, bxCNTc) = 1;
                            dcor(bxCNTr, bxCNTc) = max(max(Xcorr));
                        end
                    end
    ...
    delx = (smoothn(delx,0.01,'robust'));
    dely = (smoothn(dely,0.1,'robust'));
...
%% Adding back in the initial guess (because the images were warped by this amount before the PIV routine ran)
UGuess_GS = interp2(X1,Y1,UGuess,X,Y);
VGuess_GS = interp2(X1,Y1,VGuess,X,Y);
...
CompVel.delta_x = delx + UGuess_GS; % Final Result: vectors going downwind (from left to right) are positive
CompVel.delta_z = - dely - VGuess_GS; % Final Result: vectors going away from surface (upward) are positive

CompVel.delta_x1 = U1 + UGuess; %Final Result at resolution of the original image
CompVel.delta_z1 = -V1 - VGuess; %Final Result at resolution of the original image
