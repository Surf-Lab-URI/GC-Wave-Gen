...
IM1(isnan(IM1)) = mean(IM1(:),'omitnan');
IM2(isnan(IM2)) = mean(IM2(:),'omitnan');
IM2 = IM2*mean(IM1(:))/mean(IM2(:));
        ...
        for r = y % Loop in row: y-coordinate of interrogation window
            ...
            if VALID_BLOCK % Whether the PIV can be done (not masked)
                ...
                try
                    ...
                    % % % xIM2 = (bdryL:bdryR) + forwardx;
                    % % % xIM2_cond = ones(length(xIM2));
                    % % % bxB_x_NaN = [];
                    % % % if sum(xIM2<1)>0
                    % % %     xIM2_cond = xIM2>0;
                    % % %     bxB_x_NaN = ones(length(xIM2),abs((min(xIM2)-1)))*mean(IM2(10001:10500),'omitnan');
                    % % % end
                    % % % yIM2 = (bdryT:bdryB) + forwardy;
                    % % % yIM2_cond = ones(length(yIM2));
                    % % % bxB_y_NaN = [];
                    % % % if sum(yIM2<1)>0
                    % % %     yIM2_cond =yIM2>0;
                    % % %     bxB_y_NaN = nan(length(yIM2),abs((min(yIM2)-1)));
                    % % % end
                    % % % bxB = [bxB_x_NaN , bxB_y_NaN , IM2( yIM2, xIM2(xIM2_cond) )];
...
%% Position of the NaN
NOTnan = ones(size(delx,1),size(delx,2));
NOTnan(isnan(delx)) = 0;
...
CompVel.IW = IW(end); % Final Interrogation Window
