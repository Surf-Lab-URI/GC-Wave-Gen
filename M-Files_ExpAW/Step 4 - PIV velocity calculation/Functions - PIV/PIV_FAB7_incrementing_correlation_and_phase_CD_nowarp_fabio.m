function [CompVel] = PIV_FAB7_incrementing_correlation_and_phase_CD_nowarp_fabio(IM1, IM2, Mask, MaskOE, IntrWndw, GrdSpc)
...
delx_int=smoothn(delx,0,'robust');
dely_int=smoothn(dely,0,'robust');

delx_ints=smoothn(delx,'robust');
dely_ints=smoothn(dely,'robust');

%-------------- Median filter --------------%
% This substitutes previous local filtering avoiding loops
...
MEDX = medfilt2(smoothn(DELX,0,'robust'),[3 3],'symmetric').*MASK;
MEDY = medfilt2(smoothn(DELY,0,'robust'),[3 3],'symmetric').*MASK;
STDX = stdfilt(DELX);
STDY = stdfilt(DELY);
