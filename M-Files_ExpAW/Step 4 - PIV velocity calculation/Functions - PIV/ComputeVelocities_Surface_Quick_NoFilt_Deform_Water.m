function [CompVel] =  ComputeVelocities_Surface_Quick_NoFilt_Deform_Water(PIV1, PIV2, Mask1, Mask2, IntrWndw, GrdSpc)
...
for lvl = 1:number_of_levels-1; % First level
    ...
    if h < IW
        y = h/2; % y-coordinate of center of interrogation windows
    end
    ...
    for r = y % Loop in row: y-coordinate of center of interrogation window
        ...
        if VALID_BLOCK % Whether the PIV can be done (not masked)
            ...
            try
                ...
                % Sub-window demeaned, windowed, and FFTs
                bxAmm = bxA - mean(bxA(:),'omitmissing');
                bxBmm = bxB - mean(bxB(:),'omitmissing');
...
[X,Y] = meshgrid(x, y);
    %Interpolation of velocity to image resolution
if(size(delx_s,1)) > 1
    U1 = interp2(X,Y,delx_s,X1,Y1,'*spline'); 
    V1 = interp2(X,Y,dely_s,X1,Y1,'*spline');
else
    u1 = interp1(x,delx_s,1:w);
    v1 = interp1(x,dely_s,1:w);
    U1 = zeros(h,w);
    V1 = zeros(h,w);
    for jj = 1:h
        U1(jj,:) = u1;
